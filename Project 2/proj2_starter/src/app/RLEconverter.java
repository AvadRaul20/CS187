...
public String compressLine(String line, char[] fileChars){
   ...
   int totalCount = 1;
   int i;
   if(fileChars[0] != line.charAt(0))res += Integer.toString(0) + ",";
   ...
   for(i = 0; i < line.length(); i++){
      ...
      if(i == line.length()-1){
         res += Integer.toString(totalCount);
      }
      else{
         res += Integer.toString(totalCount) + ",";
         totalCount = 1;
  ...
  public String[] compressLines(String[] lines){
   String[] compressed = null;
      ...
      String primaryChar = lines[0];
      char[] characters = new char[2];
      characters[0] = primaryChar.charAt(0);

      for(int i = 0; i < primaryChar.length(); i++){
         if(characters[0] != primaryChar.charAt(i)){
            characters[1] = primaryChar.charAt(i);
            break;
         }
...
public String getCompressedFileStr(String[] compressed, char[] fileChars) {
   ...
   */
   public String decompressLine(String line, char[] fileChars){
      ...
      String[] decompressedString = line.split(",");
      int[] decompressedIntegers = new int[decompressedString.length];

      for(int i = 0; i < decompressedIntegers.length; i++){
         decompressedIntegers[i] = Integer.parseInt(decompressedString[i]);
      }

      for(int i = 0; i < decompressedIntegers.length; i++){
         for(int j = 0; j < decompressedIntegers[i]; j++){
            if(i%2 == 0){
            res += fileChars[0];
            }
            else{
  ...
  public String[] decompressLines(String[] lines){
   ...
   chars[0] = lines[1].charAt(0);
   for( int i = 0; i < lines[1].length(); i++){
       if(lines[1].charAt(i) != chars[0]){
          chars[1] = lines[1].charAt(i);
          break;
       }
   }
    for(int i = 0; i < lines.length; i++){
